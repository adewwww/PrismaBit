document.addEventListener('DOMContentLoaded', () => {
  ...
  if (headerNav) {
    ...
    const handleScroll = () => {
      ...
      if (scrollTop > 50) {
        headerNav.classList.add('bg-white/10', 'backdrop-blur-xl', 'shadow-md', 'border-white/20');
        headerNav.classList.remove('bg-slate-50/80', 'backdrop-blur-sm', 'border-slate-200');
      } else {
        // Revert to the default, less transparent style when at the top of the page.
        headerNav.classList.add('bg-slate-50/80', 'backdrop-blur-sm', 'border-slate-200');
        headerNav.classList.remove('bg-white/10', 'backdrop-blur-xl', 'shadow-md', 'border-white/20');
      }
